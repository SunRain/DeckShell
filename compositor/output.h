...
// Note: Q_DECLARE_LOGGING_CATEGORY(qLcLayerShell) not present in reference file
...
public:
    ...
    static Output *createPrimary(WOutput *output, QQmlEngine *engine, QObject *parent = nullptr); // Note: named createPrimary in target, create in reference
    ...
    // // Note: Window placement methods not present in reference
    ...
    // // Note: Scaling and positioning not present in reference

    // // Note: Popup surface handling not present in reference
...
public Q_SLOTS:
    void enable();
    void updatePrimaryOutputHardwareLayers(); // Note: named updatePrimaryOutputHardwareLayers in target, updateOutputHardwareLayers in reference
...
private:
    ...
    // void layoutLayerSurface(SurfaceWrapper *surface); // Note: named layoutLayerSurface in target, arrangeLayerSurface in reference
    // void layoutLayerSurfaces(); // Note: named layoutLayerSurfaces in target, arrangeLayerSurfaces in reference
    // void layoutNonLayerSurface(SurfaceWrapper *surface, const QSizeF &sizeDiff); // Note: named layoutNonLayerSurface in target, arrangeNonLayerSurface in reference
    // void layoutPopupSurface(SurfaceWrapper *surface); // Note: named layoutPopupSurface in target, arrangePopupSurface in reference
    // void layoutNonLayerSurfaces(); // Note: named layoutNonLayerSurfaces in target, arrangeNonLayerSurfaces in reference
    // void layoutAllSurfaces(); // Note: named layoutAllSurfaces in target, arrangeAllSurfaces in reference
    ...
    QPointer<QQuickItem> m_menuBar; // Note: not present in reference
