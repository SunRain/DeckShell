...
#include "config/treelandconfig.hpp"
...
TreelandConfig::TreelandConfig()
    ...
    , m_cursorBlink(m_dconfig->value("cursorBlink", true).toBool())
    , m_cursorBlinkTime(m_dconfig->value("cursorBlinkTime", 1200).toUInt())
    , m_dndDragThreshold(m_dconfig->value("dndDragThreshold", 8).toUInt())
    , m_doubleClickDistance(m_dconfig->value("doubleClickDistance", 5).toUInt())
    , m_doubleClickTime(m_dconfig->value("doubleClickTime", 250).toUInt())
    , m_lostScreen(m_dconfig->value("lostScreen", "MoveToPrimary").toString())
    , m_preferDark(m_dconfig->value("preferDark", false).toBool())
    , m_showOnLock(m_dconfig->value("showOnLock", false).toBool())
    , m_themeName(m_dconfig->value("themeName", "").toString())
    ...
    , m_cursorSize(m_dconfig->value("cursorSize", 24).toUInt())
...
bool TreelandConfig::cursorBlink() const
{
    return m_cursorBlink;
}

void TreelandConfig::setCursorBlink(bool newCursorBlink)
{
    if (m_cursorBlink == newCursorBlink)
        return;
    m_cursorBlink = newCursorBlink;
    m_dconfig->setValue("cursorBlink", QVariant::fromValue(m_cursorBlink));
    Q_EMIT cursorBlinkChanged();
}

uint TreelandConfig::cursorBlinkTime() const
{
    return m_cursorBlinkTime;
}

void TreelandConfig::setCursorBlinkTime(uint newCursorBlinkTime)
{
    if (m_cursorBlinkTime == newCursorBlinkTime)
        return;
    m_cursorBlinkTime = newCursorBlinkTime;
    m_dconfig->setValue("cursorBlinkTime", QVariant::fromValue(m_cursorBlinkTime));
    Q_EMIT cursorBlinkTimeChanged();
}

uint TreelandConfig::dndDragThreshold() const
{
    return m_dndDragThreshold;
}

void TreelandConfig::setDndDragThreshold(uint newDndDragThreshold)
{
    if (m_dndDragThreshold == newDndDragThreshold)
        return;
    m_dndDragThreshold = newDndDragThreshold;
    m_dconfig->setValue("dndDragThreshold", QVariant::fromValue(m_dndDragThreshold));
    Q_EMIT dndDragThresholdChanged();
}

uint TreelandConfig::doubleClickDistance() const
{
    return m_doubleClickDistance;
}

void TreelandConfig::setDoubleClickDistance(uint newDoubleClickDistance)
{
    if (m_doubleClickDistance == newDoubleClickDistance)
        return;
    m_doubleClickDistance = newDoubleClickDistance;
    m_dconfig->setValue("doubleClickDistance", QVariant::fromValue(m_doubleClickDistance));
    Q_EMIT doubleClickDistanceChanged();
}

uint TreelandConfig::doubleClickTime() const
{
    return m_doubleClickTime;
}

void TreelandConfig::setDoubleClickTime(uint newDoubleClickTime)
{
    if (m_doubleClickTime == newDoubleClickTime)
        return;
    m_doubleClickTime = newDoubleClickTime;
    m_dconfig->setValue("doubleClickTime", QVariant::fromValue(m_doubleClickTime));
    Q_EMIT doubleClickTimeChanged();
}

QString TreelandConfig::lostScreen()
{
    m_lostScreen = m_dconfig->value("lostScreen", "MoveToPrimary").toString();
    return m_lostScreen;
}

void TreelandConfig::setLostScreen(const QString &newLostScreen)
{
    if (m_lostScreen == newLostScreen)
        return;
    m_dconfig->setValue("lostScreen", newLostScreen);
    m_lostScreen = newLostScreen;
    Q_EMIT lostScreenChanged();
}

bool TreelandConfig::preferDark() const
{
    return m_preferDark;
}

void TreelandConfig::setPreferDark(bool newPreferDark)
{
    if (m_preferDark == newPreferDark)
        return;
    m_preferDark = newPreferDark;
    m_dconfig->setValue("preferDark", QVariant::fromValue(m_preferDark));
    Q_EMIT preferDarkChanged();
}

bool TreelandConfig::showOnLock() const
{
    return m_showOnLock;
}

void TreelandConfig::setShowOnLock(bool newShowOnLock)
{
    if (m_showOnLock == newShowOnLock)
        return;
    m_showOnLock = newShowOnLock;
    m_dconfig->setValue("showOnLock", QVariant::fromValue(m_showOnLock));
    Q_EMIT showOnLockChanged();
}

QString TreelandConfig::themeName()
{
    m_themeName = m_dconfig->value("themeName", "").toString();
    return m_themeName;
}

void TreelandConfig::setThemeName(const QString &newThemeName)
{
    if (m_themeName == newThemeName)
        return;
    m_dconfig->setValue("themeName", newThemeName);
...
void TreelandConfig::setCursorSize(uint size)
{
...
uint TreelandConfig::cursorSize()
{
    int value = m_dconfig->value("cursorSize", 24).toUInt();
    m_cursorSize = value;

    // qCDebug(treelandConfig) << "cursorSize: " << QString::number(m_cursorSize);
