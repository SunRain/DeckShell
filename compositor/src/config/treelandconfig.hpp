    ...
    , public DTK_CORE_NAMESPACE::DSingleton<TreelandConfig>
{
    ...
    Q_PROPERTY(uint cursorBlinkTime READ cursorBlinkTime WRITE setCursorBlinkTime NOTIFY cursorBlinkTimeChanged FINAL)
    Q_PROPERTY(uint dndDragThreshold READ dndDragThreshold WRITE setDndDragThreshold NOTIFY dndDragThresholdChanged FINAL)
    Q_PROPERTY(uint doubleClickDistance READ doubleClickDistance WRITE setDoubleClickDistance NOTIFY doubleClickDistanceChanged FINAL)
    Q_PROPERTY(uint doubleClickTime READ doubleClickTime WRITE setDoubleClickTime NOTIFY doubleClickTimeChanged FINAL)
    Q_PROPERTY(QString lostScreen READ lostScreen WRITE setLostScreen NOTIFY lostScreenChanged FINAL)
    Q_PROPERTY(bool preferDark READ preferDark WRITE setPreferDark NOTIFY preferDarkChanged FINAL)
    Q_PROPERTY(bool showOnLock READ showOnLock WRITE setShowOnLock NOTIFY showOnLockChanged FINAL)
    ...
    Q_PROPERTY(uint cursorSize READ cursorSize WRITE setCursorSize NOTIFY cursorSizeChanged FINAL)
...
public:
  ...
  void resetFont() {
        ...
        //         m_config.loadRelaxed()->reset(QStringLiteral("font"));
        //     });
        // }
        m_dconfig->reset(QStringLiteral("font"));
    }
    void resetMonoFont() {
        // if (auto config = m_config.loadRelaxed()) {
        //     QMetaObject::invokeMethod(config, [this]() {
        //         m_config.loadRelaxed()->reset(QStringLiteral("monoFont"));
        //     });
        // }
        m_dconfig->reset(QStringLiteral("monoFont"));
    }
    ...
    void setCursorBlink(bool newCursorBlink);

    uint cursorBlinkTime() const;
    void setCursorBlinkTime(uint newCursorBlinkTime);

    uint dndDragThreshold() const;
    void setDndDragThreshold(uint newDndDragThreshold);

    uint doubleClickDistance() const;
    void setDoubleClickDistance(uint newDoubleClickDistance);

    uint doubleClickTime() const;
    void setDoubleClickTime(uint newDoubleClickTime);

    QString lostScreen();
    void setLostScreen(const QString &newLostScreen);

    bool preferDark() const;
    void setPreferDark(bool newPreferDark);

    bool showOnLock() const;
    void setShowOnLock(bool newShowOnLock);
...
private:
    ...
    bool m_cursorBlink;
    uint m_cursorBlinkTime;
    uint m_dndDragThreshold;
    uint m_doubleClickDistance;
    uint m_doubleClickTime;
    QString m_lostScreen;
    bool m_preferDark;
    bool m_showOnLock;
    QString m_themeName;
